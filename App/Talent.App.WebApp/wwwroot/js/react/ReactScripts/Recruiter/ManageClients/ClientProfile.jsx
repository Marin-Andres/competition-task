...
class ClientProfileModal extends React.Component {
    ...
    loadData(id) {
        if (id != undefined) {
            ...
            $.ajax({
                url: 'https://talentservicesprofile-fph9ckfqh4gseace.australiasoutheast-01.azurewebsites.net/profile/profile/getEmployerProfile?id=' + id + '&role=' + 'employer',
                headers: {
    ...
    saveData() {
        var cookies = Cookies.get('talentAuthToken');
        $.ajax({
            url: 'https://talentservicesprofile-fph9ckfqh4gseace.australiasoutheast-01.azurewebsites.net/profile/profile/saveClientProfile',
            headers: {
                'Authorization': 'Bearer ' + cookies,
                'Content-Type': 'application/json'
            },
